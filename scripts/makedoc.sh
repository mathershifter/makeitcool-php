#!/bin/bash
# $Id: makedoc.sh 802 2010-03-24 23:09:19Z jmather5 $ 
...
#  */
DIRECTORIES=$PWD/library,$PWD/tutorials
...
#  * @var               string FILES
#  */
FILES=$PWD/README,$PWD/INSTALL,$PWD/CHANGELOG,$PWD/MIT-LICENSE
...
# make documentation
"$PATH_PHPDOC" -f "$FILES" -d "$DIRECTORIES" -i "$FILE_IGNORE" -t "$PATH_DOCS" \
-ti "$TITLE" -dn $PACKAGES -o $OUTPUTFORMAT:$CONVERTER:$TEMPLATE -pp $PRIVATE
