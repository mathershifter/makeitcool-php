...
/**
 * Mic_Rrd_Parser_Fetch
 *
 * @category   Mic
 * @package    Mic_Rrd
 */
class Mic_Rrd_Parser_Fetch extends Mic_Rrd_Parser_Abstract
{
        ...
        foreach ($response as $line) {
            ...
            if (count($keys) == 1 && preg_match('/^\w+/', $line)) {
                ...
                if (($_record = @array_combine($keys, $values)) === false) {
                    require_once 'Mic/Rrd/Parser/Exception.php';
                    throw new Mic_Rrd_Parser_Exception("Parser error: field count does not match value count");
                }
